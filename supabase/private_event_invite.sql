-- =============================================================
-- Private events shareable via member referral links
-- Run in the Supabase SQL editor (after prospect_registration.sql).
-- Safe to re-run.
--
-- Rule: a published PRIVATE event behaves exactly like a public one
-- when the visitor arrives with a valid member referral code
-- (/invite/<eventId>?ref=CODE). Without one, anonymous visitors still
-- get "Invite not available". Logged-in members can already read
-- private events through RLS; this only opens the anonymous path.
-- =============================================================

/*
 * get_invite_event: single gate for the public invite/registration surfaces.
 * Returns { event, speakers } for a published event when it is public, or
 * private + valid ref code. Returns null otherwise. SECURITY DEFINER so
 * anon can load private events + their speakers without widening RLS.
 */
...
begin
  ...
  if v_event.visibility <> 'public' and not v_ref_valid then
    return null;
  end if;
...
/*
 * register_prospect_for_event: same as prospect_registration.sql, with one
 * change — the referral is resolved BEFORE the visibility gate, and a valid
 * referral opens registration for private events.
 */
...
begin
  ...
  -- Resolve referral / inviter attribution first: a valid member referral
  -- is what opens a private event.
  if p_ref_code is not null and length(trim(p_ref_code)) > 0 then
  ...
  if v_event.status <> 'published'
     or (v_event.visibility <> 'public' and v_referral_id is null) then
